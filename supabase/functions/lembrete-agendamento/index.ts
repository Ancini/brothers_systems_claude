...
function montarEmail(nome: string, nomeEstabelecimento: string, horario: string | null) {
  ...
  const texto =
    `Ola ${nome}, tudo bem? Lembre-se que hoje você possui horário marcado na barbearia ${nomeEstabelecimento}, às ${formatarHorario(horario)}.\n\n` +
...
Deno.serve(async () => {
  ...
  for (const agendamento of agendamentos ?? []) {
    if (!agendamento.email_usuario) {
      console.warn(`Agendamento ${agendamento.id_agendamento} sem e-mail cadastrado, pulando.`);
      continue;
    }
